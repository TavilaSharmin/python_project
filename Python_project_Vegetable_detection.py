# This python program captures real-time image of the vegetable using webcam and 
# detects the vegetable using OpenCV with a trained dataset
# the program is tranined for 5 different vegetables: Carrot, Bell_pepper, Tomato, Potato, and Broccoli
...
class MyWindow:       #main class 
    def __init__(self, win):        # initialize method
        ...
        self.t2=Entry()
        # create buttons
        self.btn0 = Button(win, text='Sell') #if item being sold
        self.btn1 = Button(win, text='Buy')  #if item being bought
        self.btn2=Button(win, text='Reset')  #if the inputs need to be reset
        ...
        # if button being pressed, call associated function
        self.b0=Button(win, text='Sell', command=self.sell) #call sell function
        self.b1=Button(win, text='Buy', command=self.buy) #call buy function
        self.b2=Button(win, text='Reset') #call reset function
        self.b2.bind('<Button-1>', self.reset)
        
        #positioning elements
        self.b0.place(x=100, y=150)
        self.b1.place(x=160, y=150)
        self.b2.place(x=220, y=150)
        self.lbl3.place(x=20, y=220)
        self.lbl4.place(x=30, y=190)

        #creating the message box   
        self.t4 = Text(win, x=20, y=250, height=5, width=20,bg='yellow') 
    ...
    def imgDetection(self):
        ...
        while True:
            ...
            if k%256 == 27:
                ...
                # when SPACE pressed, image being captured
                ...
                # update current stock of the item detected
                sql_update = "UPDATE tbl_item_price SET current_stock_in_lb=%s WHERE item_name=%s"
                val =  (newstock, itemName)
                ...
                mycursor.close ()
                mydb.close()
                result = result + "\nYour Updated Account Balance($): "+ str(round((updatedAmount),2))
                #display message
    ...
    def sell(self):
        ...
        self.arg=1 #flag variable for Sell
    ...
    def buy(self):
        ...
        num1=int(self.t1.get()) #get the user inputs
        num2=float(self.t2.get())
        self.arg=0 #flag variable for Buy
        self.imgDetection() # calling the imgDetection function
            
    # function to erase the user iputs if Reset is pressed
